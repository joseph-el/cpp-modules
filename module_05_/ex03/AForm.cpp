...
AForm::AForm() : name("AForm"), grade_sign(0), grade_execute(0) , is_signed(false) {
    std::cout << "Default called !" << std::endl;
}

AForm::AForm(const std::string &_name, const short x, const short y) : name(_name), grade_sign(x), grade_execute(y) , is_signed(false) {
    std::cout << "by setting called !" << std::endl;
    
    (grade_sign > 150) ? throw AForm::GradeTooLowException() : 0;
    (grade_sign < 1) ? throw AForm::GradeTooHighException()  : 0;
	
	(grade_execute > 150) ? throw AForm::GradeTooLowException() : 0;
	(grade_execute < 1) ? throw AForm::GradeTooHighException()  : 0;
}
